...
const InviteNotificationCard = ({ from, topicid, topicname, notifyid, read }: InviteNotificationCardProps) => {
    ...
    return (
        <>
            <div className={`flex flex-col gap-3 px-3 py-2 ${read === true && `bg-green-50`}`}>
                <div className='text-sm'>
                    You got a collab invitation from <Link className='text-blue-400 underline' href={`/u/${from}`} target='_blank'>{from}</Link> for <Link className='text-green-500 underline' href={`/topic/${topicid}`} target='_blank'>{topicname}</Link>
                </div>
